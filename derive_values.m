...
function [m_wall,m_air,m_glass,A_out,A_in] = derive_values (...
    win_area, in_H, in_L, in_W,wall_thick,wall_density, air_density, win_thick, win_density)
    ...
    m_glass = win_area*win_thick*win_density;
    ...
    A_out = 2*((in_W+2*wall_thick)*(in_H+wall_thick))+((in_L+2*wall_thick) ...
        *(in_H+wall_thick)) + ((in_L+2*wall_thick)*(in_H+wall_thick)) + ...
        ((in_L+2*wall_thick)*(in_W+2*wall_thick)) - win_area; % m^2
    A_in = (2*(in_L*in_H + in_W*in_H + in_L*in_W)) - win_area; % m^2
end